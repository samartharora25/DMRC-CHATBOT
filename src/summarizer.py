...
You are an expert HR and legal document summarizer. Your job is to generate accurate, markdown-formatted summaries of HR policy content. 
Your summary must:
- Compress the input text to **exactly 50% of its original length** (in words or tokens).
- **Preserve all factual content**, legal clauses, logic structures, and procedures from the original.
- Maintain fidelity to names of sections, approval flows, if/else logic, fee structures, and time-based actions.
- Avoid repeating content unless explicitly stated.
- Use markdown formatting to clearly separate policy rules, conditions, exceptions, and tables.
- Do not hallucinate or omit any point.

Format your output using:
- Headings (`##`, `###`) for each major section
- Bulleted lists for steps, rules, exceptions
- Markdown tables for fee charts or annexures
- Redact any PII (emails, names, IDs) with `[REDACTED]`.
...
- Summarize into a markdown document.
- Sections to extract (if present): 
...
- Ensure section titles like `Overstay Charges`, `Improper Use`, `Director's Overrides` appear as headers if found in the content.
- Do NOT rewrite or expand the meaning.
- Only compress and reformat.
- Output length must be ~50% of original content length.
...
class HRPolicySummarizer:
    ...
    def summarize_batch(self, documents: List[Document], on_result_callback=None) -> None:
        """Process multiple documents with optional callback for individual results"""
        console.print(f"\n📝 [cyan]Processing {len(documents)} documents...[/cyan]")
        
        for i, doc in enumerate(documents, 1):
            console.print(f"Processing document {i}/{len(documents)}")
            sum_result = self.summarize_document(doc)
            
            # Call callback with individual result if provided
            if on_result_callback:
                on_result_callback(sum_result, i, len(documents))
